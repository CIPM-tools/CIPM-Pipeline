...
import cipm.consistency.base.shared.util.ExtendedEqualityHelper;
...
public abstract class AbstractPipelineTestBase extends VsumManagerTestBase {
	...
	protected List<Tree<ServiceCallRecord>> monitoringDataFromStrings(String[] data) {
		...
		Tree<ServiceCallRecord> currentTree = new Tree<>(parseRecord(data[0], true));
		...
		for (int i = 1; i < data.length; i++) {
			if (data[i].length() > 0 && !data[i].startsWith("#")) {
				...
				if (nIndent > currentIndent) {
					currentNode = currentNode.addChildren(parseRecord(data[i], false));
					currentIndent = nIndent;
				} else if (nIndent <= currentIndent) {
					if (nIndent == 0) {
						out.add(currentTree);
						currentTree = new Tree<>(parseRecord(data[i], true));
						...
						// they are equal
						currentNode.addChildren(parseRecord(data[i], false));
					}
				}
			}
		}
	...
	private ServiceCallRecord parseRecord(String record, boolean entry) {
		if (entry) {
			return parseRecord(record, "<not set>");
		} else {
			return parseRecord(record, "");
		}
	}

	private ServiceCallRecord parseRecord(String record, String serviceCallerId) {
		...
		if (hostIdMatcher.find()) {
			...
			if (externalCallSplit.length == 2 && dataSplit.length == 2) {
				ServiceCallRecord rec = new ServiceCallRecord("", "", hostIdMatcher.group(1), "", externalCallSplit[1],
						"", serviceCallerId, externalCallSplit[0], "", System.currentTimeMillis(),
				...
				if (parameterMatcher.find()) {
					ServiceCallRecord rec = new ServiceCallRecord("", "", hostIdMatcher.group(1), "",
							externalCallSplit[1], parameterMatcher.group(1), serviceCallerId, externalCallSplit[0], "",
