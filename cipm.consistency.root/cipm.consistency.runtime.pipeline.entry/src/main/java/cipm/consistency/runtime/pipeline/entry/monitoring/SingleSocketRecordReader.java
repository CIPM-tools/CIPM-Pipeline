 ...
 ***************************************************************************/
package cipm.consistency.runtime.pipeline.entry.monitoring;
...
public class SingleSocketRecordReader {
	...
	public SingleSocketRecordReader(final IRecordReceivedListener listener) {
	...
	private boolean deserializeRecord(final int clazzId, final ByteBuffer buffer) {
		...
		final long loggingTimestamp = buffer.getLong(); // NOPMD (timestamp must be read before checking the buffer for
														// record size)
		...
		try {
			final IMonitoringRecord record = recordFactory
					.create(DefaultValueDeserializer.create(buffer, this.stringRegistryWrapper));
			record.setLoggingTimestamp(loggingTimestamp);

			this.listener.onRecordReceived(record);
		} catch (final RecordInstantiationException ex) {
			log.warning("Failed to create: " + recordClassName);
		}
