...
public class SyncHostConnectionCreationRoutine extends AbstractRepairRoutineRealization {
  ...
  private static class ActionUserExecution extends AbstractRepairRoutineRealization.UserExecution {
    ...
    public EObject getCorrepondenceSourceContainerTo(final RuntimeResourceContainerConnection affectedObject, final ResourceContainer containerFrom) {
      RuntimeResourceContainer _containerTo = affectedObject.getContainerTo();
      return _containerTo;
    }
    
    public EObject getCorrepondenceSourceParent(final RuntimeResourceContainerConnection affectedObject, final ResourceContainer containerFrom, final ResourceContainer containerTo) {
      EObject _eContainer = affectedObject.eContainer();
      return _eContainer;
    }
    
    public void callRoutine1(final RuntimeResourceContainerConnection affectedObject, final ResourceContainer containerFrom, final ResourceContainer containerTo, final ResourceEnvironment parent, @Extension final RoutinesFacade _routinesFacade) {
      if ((((containerFrom != null) && (containerTo != null)) && (parent != null))) {
        final Function1<LinkingResource, Boolean> _function = new Function1<LinkingResource, Boolean>() {
          public Boolean apply(final LinkingResource it) {
            return Boolean.valueOf((it.getConnectedResourceContainers_LinkingResource().contains(containerFrom) && it.getConnectedResourceContainers_LinkingResource().contains(containerTo)));
          }
        };
        final LinkingResource corrLink = IterableExtensions.<LinkingResource>findFirst(parent.getLinkingResources__ResourceEnvironment(), _function);
        if ((corrLink == null)) {
  ...
  public SyncHostConnectionCreationRoutine(final RoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final RuntimeResourceContainerConnection affectedObject) {
    super(routinesFacade, reactionExecutionState, calledBy);
    this.userExecution = new mir.routines.remToPCM.SyncHostConnectionCreationRoutine.ActionUserExecution(getExecutionState(), this);
  ...
  protected boolean executeRoutine() throws IOException {
    ...
    org.palladiosimulator.pcm.resourceenvironment.ResourceContainer containerTo = getCorrespondingElement(
    	userExecution.getCorrepondenceSourceContainerTo(affectedObject, containerFrom), // correspondence source supplier
    	org.palladiosimulator.pcm.resourceenvironment.ResourceContainer.class,
    	(org.palladiosimulator.pcm.resourceenvironment.ResourceContainer _element) -> true, // correspondence precondition checker
    	null, 
    	false // asserted
    ...
    org.palladiosimulator.pcm.resourceenvironment.ResourceEnvironment parent = getCorrespondingElement(
    	userExecution.getCorrepondenceSourceParent(affectedObject, containerFrom, containerTo), // correspondence source supplier
