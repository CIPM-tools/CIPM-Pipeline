...
public class JFastEMD {
	 ...
	 * Histogram Metric for Improved SIFT Matching Ofir Pele, Michael Werman ECCV
	 * 2008
	 *
	 ...
	 * Distances Ofir Pele, Michael Werman ICCV 2009
	 *
		...
		for (int i = 0; i < c.size(); i++) {
			...
			for (Edge it : c.get(i)) {
				if (nodesNewNames.get(it._to) != REMOVE_NODE_FLAG) {
					cc.get(nodesNewNames.get(i)).add(new Edge(nodesNewNames.get(it._to), it._cost));
				}
			}
		}
